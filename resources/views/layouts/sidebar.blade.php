        ...
        <div class="mt-5">
            <div class="flex flex-col space-y-2">
                ...
                <div class="h-20 flex items-center justify-center text-center">
                    <a href="{{ route('menus.index') }}" class="rounded hover:bg-blue-600 px-4 py-2 border-b w-[500px] text-center">
                        Menu Makanan
                    </a>
                </div>

                <div class="h-20 flex items-center justify-center text-center">
                    <a href="{{ route('myorders.index') }}" class="rounded hover:bg-blue-600 px-4 py-2 border-b w-[500px] text-center">
                        Pesanan Saya
                    </a>
                </div>
                ...
                @if(Auth::check() && Auth::user()->isAdmin())
                    <div class="h-20 flex items-center justify-center text-center">
                        <a href="{{ route('orders.index') }}" class="rounded hover:bg-blue-600 px-4 py-2 border-b w-[500px] text-center {{ request()->is('orders') ? 'bg-blue-600' : '' }}">
                            Daftar Pesanan
                        </a>
                    </div>
                    <div class="h-20 flex items-center justify-center text-center">
                        <a href="{{ route('reports.index') }}" class="rounded hover:bg-blue-600 px-4 py-2 border-b w-[500px] text-center">
                            Laporan
                        </a>
                    </div>
                @endif
    ...
    <div :class="{'ml-48': open, 'ml-0': !open}" class="flex-1 transition-all duration-300">
        <nav class="bg-white shadow mb-4 flex justify-between items-center p-3" style="min-height: 64px;">
            ...
            <div class="relative ml-4 flex items-center">
                <div class="mr-2">
                    <span class="font-semibold text-gray-800">{{ Auth::check() ? Auth::user()->name : 'Guest' }}</span>
                    <div class="text-sm text-gray-600">
                        {{ Auth::check() && Auth::user()->isAdmin() ? 'Admin' : 'Customer' }}
                    </div>
                </div>
                <button type="button" @click="isOpen = !isOpen" class="flex items-center rounded-full focus:outline-none mr-2">
                    <img class="h-10 w-12 rounded-full" src="{{ asset('img/profile.png') }}" alt="Profile Picture">
                </button>
